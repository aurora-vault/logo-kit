      ...
      <div v-for="sh in shapes" :key="sh.id" class="flex flex-col items-center gap-1">
        <div class="relative aspect-square w-full">
          <img
            v-if="img"
            :src="img.url"
            class="h-full w-full border border-slate-700 object-cover"
            :style="{ borderRadius: sh.r }"
            alt="pwa"
          />
          <div v-else class="h-full w-full border border-dashed border-slate-600" :style="{ borderRadius: sh.r }"></div>
          <div class="pointer-events-none absolute inset-[10%] rounded-full border border-dashed border-yellow-400/60"></div>
        </div>
        <span class="text-[10px] text-slate-500">{{ sh.name }}</span>
      </div>
    </div>
    <p class="text-[10px] text-slate-500">虚线圈 = safe zone。maskable 文件主体应已在圈内,任何形状都不裁到。</p>
  </div>
</template>
