...
<template>
  <div class="space-y-3">
    ...
    <div :class="isBoth ? 'grid grid-cols-2 gap-2' : ''">
      <div v-if="showDark" class="rounded-md border border-white/10 p-2.5" :style="{ background: bgDark }">
        ...
        <div class="mt-2 flex gap-2 text-[9px] opacity-60" :style="{ color: textOn(bgDark) }"><span>关于</span><span>归档</span><span>RSS</span></div>
      </div>
      <div v-if="showLight" class="rounded-md border border-black/10 p-2.5" :style="{ background: bgLight }">
